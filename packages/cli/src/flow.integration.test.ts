...
const MANIFEST = join(
  fileURLToPath(
    new URL("../../../examples/skill.manifest.json", import.meta.url),
  ),
);
...
describe("agentos CLI distribution flow", () => {
  ...
  it("init → create agent → publish skill → resolve", () => {
    ...
    const createOut = JSON.parse(
      runAgentos(
        ["agent", "create", "flow-test.sui", "--wallet", "0xFLOW", "--json"],
        dir,
      ),
    );
  ...
  it("dry-run prepares transaction without requiring on-chain package", () => {
    const out = runAgentos(
      ["agent", "create", "dry.sui", "--wallet", "0x1", "--dry-run", "--json"],
      dir,
    );
  ...
  it("persists registry to disk", () => {
    ...
    expect(
      data.agents.some(
        (a: { suinsName: string }) => a.suinsName === "flow-test.sui",
      ),
    ).toBe(true);
  });
});
